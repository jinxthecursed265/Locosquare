// b, h, j, l, m, o, p, r, w

function la(){ // 我在一家高级餐厅里把一块 Twinkie 塞进了我的屁股
	with (instance_create_layer(0, 0, "Instances", obj_mustard)){
		sprite_index = mustard
		object = false}
}

function lb(){ // click the bart
	global.bartcounter = 0
	room_goto(clickthebart)
	window_set_caption("click the bart")
}

function lc(){
	
}

function ld(){ //die lit
	with (instance_create_layer(0, 0, "Instances", obj_jesus))
		sprite_index = die_lit
	window_set_caption("die lit")
}

function le(){
	
}

function lf(){
	
}

function lg(){
	
}

function lh(){ // bouncing horse
	room_goto(TheHorse)
	window_set_caption("ima bounce")
}

function li(){
	
}

function lj(){ // jesus
with (instance_create_layer(0, 0, "Instances", obj_jesus))
	sprite_index = THE_COCK_IS_WATCHING
audio_play_sound(jesas, 1, true)
window_set_caption("hello i am jesus")
}

function lk(){
	
}

function ll(){ // luigi tony hawk
	with (instance_create_layer(0, 0, "Instances", obj_videoplayer))
		videoinfo = ["luigi.mp4", 364, 360]
		window_set_caption("holy shit its luigi")
}

function lm(){ // mario minecraft
	with (instance_create_layer(0, 0, "Instances", obj_videoplayer))
		videoinfo = ["mario.mp4", 294, 220]
		window_set_caption("HELLO WE GOT SOME MORE MARIO CHARACTERS FOR YO")
}

function ln(){
	
}

function lo(){ // ow oh knee
	room_goto(ou_oni)
		audio_play_sound(ao_oni, true, true)
		window_set_caption("dude you need to run like run really fast")
}

function lp(){ // WHEN I FRICKIN DOWNLOADED PIZZA TOWER UNITED IT FRICKING INFECTED EVERYTHING LIKE I DOWNLOADED IT FROM PIZZA TOWER AND EVERYTHING I DO THE PTU DISCORD SDK WILL ALWAYS BE THERE AND ARE YOU SERIOUS!?! FUCK YOU ALL IM GONNA LEAVE THIS SHIT NOW GO FUCK YOURSELFS AND NEVER COME BACK TO INVITE ME ON THIS EVER AGAIN EVERYTHING I DO VERIFY INTEGRITY DOES NOT WORK UNINSTALL DOES NOT WORK REDOWNLOAD STEAM NOT EVEN FUCKING WORKING LIKE THE WHAT THE FUCK?!! GO FUCK YOURSELFS!
	with (instance_create_layer(0, 0, "Instances", obj_jesus))
		sprite_index = spr_WHENIFRICKINDOWNLOADEDPIZZATOWERUNITEDITFRICKINGINFECTEDEVERYTHING
		window_set_caption("WHEN I FRICKIN DOWNLOADED PIZZA TOWER UNITED IT FRICKING INFECTED EVERYTHING LIKE I DOWNLOADED IT FROM PIZZA TOWER AND EVERYTHING I DO THE PTU DISCORD SDK WILL ALWAYS BE THERE AND ARE YOU SERIOUS!?! FUCK YOU ALL IM GONNA LEAVE THIS SHIT NOW GO FUCK YOURSELFS AND NEVER COME BACK TO INVITE ME ON THIS EVER AGAIN EVERYTHING I DO VERIFY INTEGRITY DOES NOT WORK UNINSTALL DOES NOT WORK REDOWNLOAD STEAM NOT EVEN FUCKING WORKING LIKE THE WHAT THE FUCK?!! GO FUCK YOURSELFS!")
}

function lq(){
	
}

function lr(){ // rec room lmao
	with (instance_create_layer(0, 0, "Instances", obj_videoplayer))
		videoinfo = ["recroomtrailerverybad.mp4", 256, 144]
		window_set_caption("rec room")
}

function ls(){ // The entire half time show
		with (instance_create_layer(0, 0, "Instances", obj_videoplayer))
		videoinfo = ["halftimeshow.mp4", 1920, 1080]
		window_set_caption("MUSTAAAAAAAAARRRRRDDDDDDDDD")
}

function lt(){
	
}

function lu(){
	
}

function lv(){
	
}

function lw(){ // wega
	with (instance_create_layer(0, 0, "Instances", obj_jesus)){
		sprite_index = wega
		object = false}
}

function lx(){
	
}

function ly(){
	
}

function lz(){
	
}
